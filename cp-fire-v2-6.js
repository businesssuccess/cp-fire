console.log("Script cp-fire-v2-6.js loaded and starting execution.");
...
if (typeof firebase !== 'undefined' && firebase.apps.length === 0) {
  ...
  function logEvent(contactId, eventName, eventAction, contactData) {
    ...
    // Also log the event under the prospect's node, allowing multiple event actions
    const prospectEventRef = firebase.database().ref(`prospects/${contactId}/events/${eventName}`);
    const newEventActionRef = prospectEventRef.push();
    newEventActionRef.set({ eventAction, timestamp: currentDateTime }).catch((error) => {
