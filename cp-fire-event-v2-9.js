console.log("Script cp-fire-event-v2-9.js loaded and starting execution.");
...
if (typeof firebase !== 'undefined' && firebase.apps.length === 0) {
  ...
  function logEvent(contactId, eventName, eventAction, contactData) {
    ...
    const currentDateTime = new Date().toISOString().replace(/[:.]/g, '-'); // Get current date and time and sanitize for Firebase path
    ...
    // Also log the event under the prospect's node with a unique identifier for each action
    const uniqueEventActionRef = firebase.database().ref(`prospects/${contactId}/events/${eventName}/${eventAction}_${currentDateTime}`);

    uniqueEventActionRef.set({
      timestamp: currentDateTime,
      ...contactData
    }).then(() => {
