console.log("Script cp-fire-event-v2-13.js loaded and starting execution.");
...
if (typeof firebase !== 'undefined' && firebase.apps.length === 0) {
  ...
  function logEvent(contactId, eventName, eventAction, contactData) {
    ...
    // Update the events under the prospect's node
    const prospectEventRef = firebase.database().ref(`prospects/${contactId}/events/${eventName}/${eventAction}`);
    prospectEventRef.once('value').then((snapshot) => {
      if (!snapshot.exists()) {
        ...
        console.log("Event action already logged under this prospect.");
      }
    }).catch((error) => {
