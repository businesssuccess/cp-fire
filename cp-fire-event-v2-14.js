console.log("Script cp-fire-event-v2-14.js loaded and starting execution.");
...
if (typeof firebase !== 'undefined' && firebase.apps.length === 0) {
  ...
  function logEvent(contactId, eventName, eventAction, contactData) {
    ...
    // Update the events under the prospect's node using a unique key
    const prospectEventRef = firebase.database().ref(`prospects/${contactId}/events/${eventName}`);
    const newEventActionRef = prospectEventRef.push(); // Generate a new unique key for each event action
