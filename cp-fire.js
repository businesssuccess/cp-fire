<script src="https://www.gstatic.com/firebasejs/8.10.0/firebase-app.js" nonce="random123"></script>
<script src="https://www.gstatic.com/firebasejs/8.10.0/firebase-database.js" nonce="random123"></script>
<script src="https://www.gstatic.com/firebasejs/8.10.0/firebase-app-check.js" nonce="random123"></script>
<script nonce="random123">
  ...
  function logEvent(contactId, eventName, eventAction, contactData) {
    ...
    eventRef.once('value').then((snapshot) => {
      if (!snapshot.exists()) {
        eventRef.set({ ...contactData, timestamp: currentDateTime }).then(() => {
    ...
    }).catch((error) => console.error("Error checking event log:", error));
  }
  ...
  function createOrUpdateProspect(contact) {
    return new Promise((resolve, reject) => {
      ...
      Object.keys(contact).forEach(key => {
        if (!knownFields.includes(key.toLowerCase())) contactData[key] = contact[key];
      });
      ...
      prospectRef.once('value').then((snapshot) => {
        if (!snapshot.exists()) {
          ...
          Object.keys(contactData).forEach(field => {
            if (contactData[field] !== snapshot.val()[field] && contactData[field] !== 'Unknown') updates[field] = contactData[field];
          });
  ...
  // Function to store device details
  function storeDeviceDetails(emailKey, ipAddress, deviceDetails) {
    ...
    deviceRef.once('value').then((snapshot) => {
      if (!snapshot.exists()) {
        deviceRef.set(deviceDetails).then(() => console.log("Device details stored successfully."))
          .catch((error) => console.error("Error storing device details:", error));
      } else {
        deviceRef.update(deviceDetails).then(() => console.log("Device details updated successfully."))
          .catch((error) => console.error("Error updating device details:", error));
      }
    }).catch((error) => console.error("Error checking device details:", error));
  }

  // Function to process event
  function processEvent(profile, eventName, eventAction) {
    if (profile !== "Event") return;

    const contact = {{ contact | json }}; // Fetch the contact data dynamically
    const emailKey = contact.email.replace(/\./g, '-');
    ...
    console.log(`Profile: ${profile}, Event Name: ${eventName}, Event Action: ${eventAction}`);
  }
