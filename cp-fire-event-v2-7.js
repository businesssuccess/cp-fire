console.log("Script cp-fire-event-v2-7.js loaded and starting execution.");

// Ensure Firebase is loaded before running this script
if (typeof firebase !== 'undefined' && firebase.apps.length === 0) {
  console.log("Firebase is defined and apps are not initialized.");
  ...
  function storeDeviceDetails(emailKey, ipAddress, deviceDetails) {
    ...
    deviceRef.once('value').then((snapshot) => {
      if (!snapshot.exists()) {
        deviceRef.set(deviceDetails).catch((error) => {
          console.error("Error storing device details:", error);
        });
      } else {
        deviceRef.update(deviceDetails).catch((error) => {
  ...
  function logEvent(contactId, eventName, eventAction, contactData) {
    ...
    // Also log the event under the prospect's node with proper structure
    const prospectEventRef = firebase.database().ref(`prospects/${contactId}/events/${eventName}`);
    const eventActionRef = prospectEventRef.child(eventAction);

    eventActionRef.once('value').then((snapshot) => {
      if (!snapshot.exists()) {
        // Log the event action under the prospect's event name if it doesn't exist
        eventActionRef.set({ timestamp: currentDateTime }).then(() => {
          console.log("Event action logged successfully under prospect.");
        }).catch((error) => {
          console.error("Error logging event action under prospect:", error);
        });
      } else {
        console.log("Event action already exists under prospect.");
      }
    }).catch((error) => {
      console.error("Error checking event action under prospect:", error);
    });
  }

  // Function to create or update a prospect
  function createOrUpdateProspect(contact) {
    console.log("Creating or updating prospect...");
