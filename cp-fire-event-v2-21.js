console.log("Script cp-fire-event-v2-21.js loaded and starting execution.");
...
if (typeof firebase !== 'undefined' && firebase.apps.length === 0) {
  ...
  function logEvent(contactId, eventName, eventAction, contactData) {
    ...
    // Update the events under the prospect's node with combined key
    const combinedKey = `${eventName}-${eventAction}`;
    const prospectEventRef = firebase.database().ref(`prospects/${contactId}/events/${combinedKey}`);
    const uniqueKey = prospectEventRef.push().key; // Generate a unique key for each event action

    prospectEventRef.child(uniqueKey).set({
      eventAction: eventAction,
      timestamp: currentDateTime
    }).then(() => {
      console.log(`Event action ${eventAction} logged under ${combinedKey} for prospect ${contactId}.`);
    }).catch((error) => {
